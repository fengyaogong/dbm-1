...
class DBM:
    def __init__( maxEpoch, data, targets, nLabels, nHidden, nPen, wVisHid1, bLabels1, bHidden1, bVisible1, labelsHidden1, wVisHid0, bHidden0, bVisible0):
        ...
        labelsPenIncrement = np.zeors((nLabels, nPen))

        bHiddenInc = np.zeros((1, nHidden))
        bVisibleInc = np.zeros((1, nFeatures))
        bPenInc = np.zeors((1, nPen))
        bLabelsInc = np.zeros((1, nLabels))
        ...
        negativeHiddenProbs = np.random.rand((nExamples, nHidden));
    ...
    def train(:
        ...
        ################################ Start of the positive phase ######################################
                ...
                data = (data > np.random.rand(nExamples, nHidden)).astype(int)

                (positiveHiddenProbs, positivePenProbs) = #MF something
        ...
        ################################ End of positive phase ############################################
                ...
                totin = bLabels + np.dot(positivePenProbs, np.transpose(wLabPen))
                positiveLabelProbs1 = np.exp(totin)
                targetOut = positiveLabelProbs1 / (np.dot(np.sum(positiveLabelProbs1, 1), np.ones((1,nLabels))))
        ...
        ################################ Start of the negative phase ######################################
                for iter in range(5):
                    ...
                    negativeLabelProbs = negativeLabelProbs / np.dot(np.sum(negativeLabelProbs, 1), np.ones((1, nLabels)))
        ...
        ##################################### Update weights and biases ########################################
                ...
                hidmeans = np.dot(sparseDumping, hidmeans) + (1 - sparseDumping) * positiveHiddenActive / nExamples
                sparseGrads = np.dot(sparseCost, np.tile(hidmeans, (nExamples, 1)) - sparseTarget)

                penmeans = np.dot(sparseDumping, penmeans) + (1 - sparseDumping) * positivePenActive / nExamples
                ...
                visibleHiddenIncrement = momentum * visibleHiddenIncrement + \
                        eWeight * ( (positiveProducts - negativeProducts) / nExamples - \
                ...
                bHiddenInc = momentum * bHiddenInc + (eHiddenBias / nExamples) * \
                        ...
                        eHiddenBias / nExamples * np.sum(sparseGrads, 0)
                
                hiddenPenIncrement = momentum * hiddenPenIncrement + eWeight * ( \
                        (positiveProductsPen - negativeProductsPen) / nExamples - weightCost * wHidPen \
                        - np.dot(np.transpose(positiveHiddenProbs), sparseGrads2) / nExamples - \
                        np.transpose((np.dot(np.transpose(positivePenProbs), sparseGrads))) / nExamples)
                
                bPenInc = momentum * bPenInc + eWeight * (eHiddenBias / nExamples * \
                        (positivePenActive - negativePenActive)) - eHiddenBias / nExamples * np.sum(sparseGrads2)
                ...
                wHidPen = wHidPen + hiddenPenIncrement
                wLabPen = wLabPen + labelsPenIncrement

                bVisible = bVisible + bVisibleInc
                bHidden = bHidden + bHiddenInc
                bPen = bPen + bPenInc
                bLabels = bLabels + bLabelsInc
